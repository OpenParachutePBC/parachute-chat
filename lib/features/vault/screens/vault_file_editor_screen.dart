...
/// Screen for viewing and editing text files from the vault
///
/// Uses the Base server API to read and write files.
/// Supports markdown preview with frontmatter display, and
/// raw editing mode for all text files.
class VaultFileEditorScreen extends ConsumerStatefulWidget {
  final String relativePath;
  final String fileName;

  const VaultFileEditorScreen({
    super.key,
  ...
  @override
  ConsumerState<VaultFileEditorScreen> createState() => _VaultFileEditorScreenState();
}

class _VaultFileEditorScreenState extends ConsumerState<VaultFileEditorScreen> {
  String? _error;
  ...
  // Parsed content for view mode (markdown files)
  ...
  Future<void> _loadFile() async {
    try {
      ...
      final content = await service.readFile(widget.relativePath);
      ...
      _originalContent = content.content;
      _editController.text = content.content;

      // Parse frontmatter for markdown files
      if (_isMarkdown) {
        final parsed = _parseContent(content.content);
        if (mounted) {
          setState(() {
  ...
  ({Map<String, dynamic>? frontmatter, String body}) _parseContent(String content) {
    ...
    try {
      ...
      debugPrint('[VaultFileEditor] Error parsing frontmatter: $e');
    }
  ...
  Future<void> _saveFile() async {
    ...
    try {
      final service = ref.read(vaultBrowserServiceProvider);
      await service.writeFile(widget.relativePath, _editController.text);
      ...
      // Refresh vault contents in case file size changed
  ...
  Widget _buildBody(bool isDark) {
    ...
    return _buildPlainTextViewer(isDark);
  }
  ...
  Widget _buildPlainTextViewer(bool isDark) {
  ...
  Widget _buildFrontmatterContent(bool isDark) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: _frontmatter!.entries.map((entry) {
        return Padding(
